...
const ProjectCard = ({ image, title, description, link, technologies, otherClasses }) => {
    return (
        <a
            href={link}
            target="_blank"
            rel="noopener noreferrer"
            className={`border border-gray-700 mb-12 p-6 rounded-2xl flex flex-col gap-4 w-9/12 hover:scale-105 duration-300 max-sm:w-full ${otherClasses}`}
        >
            <div className="border border-gray-900 relative bg-[linear-gradient(138deg,#020024_0%,#622db1_20%,#000054_35%)] w-full h-64 overflow-hidden rounded-lg">
                <img
                    className="absolute inset-0 w-9/12 h-full object-contain max-sm:scale-150 left-24 top-12 rotate-[9deg] rounded-lg"
                    src={image}
                    alt={title}
                />
            </div>
            ...
            <div className="flex justify-between mt-1">
                <div className="flex relative min-h-[40px] w-1/2 max-sm:w-full">
                    {technologies.map((ico, key) => (
                        <span
                            key={key}
                            className="border border-gray-400 border-opacity-40 bg-black-100 rounded-full p-2 absolute"
                            style={{
                                left: `${key * 35}px`,
                                zIndex: `${key * 2}`,
                            }}
